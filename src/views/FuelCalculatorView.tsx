...
export function FuelCalculatorView({ live, onOpenStrategy = () => {} }: { live: LiveFuelEstimate | null; onOpenStrategy?: () => void }) {
  ...
  return <div className="view view--fuel" data-feedback-redact={mode === 'automatic' ? 'measured-fuel-model' : undefined}>
    ...
    {mode === 'automatic' && <Card className={`fuel-auto-card ${automaticReady ? 'is-ready' : ''}`}>
      ...
      <div><Badge tone={automaticReady ? 'positive' : 'neutral'} dot>{automaticReady ? m.automatic.ready : m.automatic.waiting}</Badge><h2>{m.automatic.title}</h2><p>{automaticReady ? m.automatic.readyCopy.replace('{count}', String(live!.fuelSamplesLiters.length)).replace('{car}', live!.carName).replace('{track}', live!.trackName) : m.automatic.waitingCopy}</p>{automaticReady && <Button variant="secondary" size="sm" onClick={onOpenStrategy}>{m.automatic.openStrategy}</Button>}</div>
