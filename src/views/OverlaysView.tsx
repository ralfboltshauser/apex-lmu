  ...
  return (
    <div className="view view--overlays">
      ...
      <div className="overlay-toolbar">
        ...
        <label className="overlay-display-select"><Monitor size={15} /><span className="sr-only">{m.toolbar.display}</span><select value={displayId} onChange={(event) => { setDisplayId(event.target.value); void persist({ displayId: event.target.value }) }} disabled={!displays.length}>{displays.length ? displays.map((display) => <option key={display.id} value={display.id}>{display.label} · {display.bounds.width}×{display.bounds.height} · {Math.round(display.scaleFactor * 100)}% · {display.rotation}°{display.primary ? ` · ${m.toolbar.primary}` : ''}</option>) : <option>{m.toolbar.noDisplays}</option>}</select></label>
