...
import { LiveDashboard, type LiveDashboardLabels } from '../components/LiveDashboard'
import type { LiveModuleId } from '../live-layout'
...
const copy = defineMessages({
  ...
  layout: { edit: 'Edit layout', done: 'Done', help: 'Reorder, resize or hide Live cards.', reset: 'Reset', resetQuestion: 'Restore the shipped Live layout?', confirmReset: 'Restore default', cancel: 'Cancel', drag: 'Drag {title} to reorder', moveBefore: 'Move {title} before', moveAfter: 'Move {title} after', makeCompact: 'Make {title} compact', makeWide: 'Make {title} wide', earlier: 'Earlier', later: 'Later', compact: 'Compact', wide: 'Wide', hideShort: 'Hide', hide: 'Hide {title}', hidden: 'Hidden cards', restore: 'Restore {title}', moved: '{title}, position {position} of {count}.', resized: '{title} is now {size}.', hiddenAnnouncement: '{title} hidden.', restored: '{title} restored.', resetAnnouncement: 'Default Live layout restored.', modules: { trackMap: 'Track map', fuel: 'Fuel', standings: 'Standings', carState: 'Car state', events: 'Events' } },
  runtime: {
    session: { practice: 'Practice', qualifying: 'Qualifying', race: 'Race', testDay: 'Test day', unknown: 'Session' },
  ...
  layout: { edit: 'Layout bearbeiten', done: 'Fertig', help: 'Live-Karten anordnen, skalieren oder ausblenden.', reset: 'Zurücksetzen', resetQuestion: 'Das ausgelieferte Live-Layout wiederherstellen?', confirmReset: 'Standard wiederherstellen', cancel: 'Abbrechen', drag: '{title} zum Anordnen ziehen', moveBefore: '{title} nach vorne verschieben', moveAfter: '{title} nach hinten verschieben', makeCompact: '{title} kompakt darstellen', makeWide: '{title} breit darstellen', earlier: 'Nach vorne', later: 'Nach hinten', compact: 'Kompakt', wide: 'Breit', hideShort: 'Ausblenden', hide: '{title} ausblenden', hidden: 'Ausgeblendete Karten', restore: '{title} wiederherstellen', moved: '{title}, Position {position} von {count}.', resized: '{title} ist jetzt {size}.', hiddenAnnouncement: '{title} ausgeblendet.', restored: '{title} wiederhergestellt.', resetAnnouncement: 'Standard-Live-Layout wiederhergestellt.', modules: { trackMap: 'Streckenkarte', fuel: 'Kraftstoff', standings: 'Wertung', carState: 'Fahrzeugzustand', events: 'Ereignisse' } },
  runtime: {
    session: { practice: 'Training', qualifying: 'Qualifying', race: 'Rennen', testDay: 'Testtag', unknown: 'Sitzung' },
...
function MeasuredLiveView({ frame, measuredTrack }: { frame: TelemetryFrame; measuredTrack: MeasuredTrackSnapshot | null }) {
  ...
  const measuredModules: Record<LiveModuleId, ReactNode> = {
    'track-map': <Card className="live-map-card live-measured-map-card">
    ...
    standings: <Card className="standings-card">
    ...
    'car-state': <Card className="car-state-card">
    ...
    events: <Card className="live-events-card">
  ...
  return (
    <div className="view view--live" data-feedback-redact="measured-live-telemetry">
      ...
      <LiveDashboard labels={m.layout as LiveDashboardLabels} renderModule={(id) => measuredModules[id]} />
    </div>
  )
}
...
export function LiveView({ source, tick, frame, measuredTrack = null, connectionMessage, onStartDemo, onTroubleshoot }: { source: LiveSource; tick: number; frame?: TelemetryFrame | null; measuredTrack?: MeasuredTrackSnapshot | null; connectionMessage?: string; onStartDemo: () => void; onTroubleshoot: () => void }) {
  ...
  const demoModules: Record<LiveModuleId, ReactNode> = {
    'track-map': <Card className="live-map-card">
      <CardHeader
  ...
  return (
    <div className="view view--live">
      ...
      <LiveDashboard labels={m.layout as LiveDashboardLabels} renderModule={(id) => demoModules[id]} />
    </div>
  )
}
