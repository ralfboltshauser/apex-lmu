  ...
  return (
    <div className="view view--live">
      <div className="live-titlebar">
        <div className="live-titlebar__identity"><Badge tone="positive" dot>{m.common.measured}</Badge><div><h1>{frame.session.track.name}</h1><p>{frame.player.car.model} · {sessionKindLabel(frame.session.kind, m)} · {conditionLabel(frame.weather.trackCondition, m)}</p></div></div>
      ...
      <div className="live-instrument-strip">
        ...
        <div className="live-speed-block"><span>{m.common.speed}</span><strong>{formatNumber(frame.player.motion.speedKph, language)}<i>{m.common.speedUnit}</i></strong><small>{m.common.gear} {formatNumber(frame.player.powertrain.gear, language)} · {formatNumber(frame.player.powertrain.rpm, language)} {m.common.rpmUnit}</small></div>
      ...
      <div className="live-layout">
        <Card className="fuel-card">
          ...
          <div className="fuel-card__primary"><div><span>{m.measured.fuelInCar}</span><strong>{formatNumber(frame.player.powertrain.fuelLiters, language, 1)} <i>{m.common.literUnit}</i></strong><small>{frame.player.powertrain.fuelPerLapEstimateLiters === null ? m.measured.baselineUnavailable : `${formatNumber(frame.player.powertrain.fuelPerLapEstimateLiters, language, 2)} ${m.common.literUnit} ${m.common.perLap}`}</small></div></div>
          <div className="fuel-card__stats">
            <Metric label={m.measured.fuelRange} value={frame.player.powertrain.lapsOfFuelEstimate === null ? '—' : formatNumber(frame.player.powertrain.lapsOfFuelEstimate, language, 1)} unit={frame.player.powertrain.lapsOfFuelEstimate === null ? undefined : m.common.laps} detail={m.measured.bridgeValue} />
            <Metric label={m.measured.virtualEnergy} value={frame.player.hybrid ? formatNumber(frame.player.hybrid.virtualEnergyPercent, language, 1) : '—'} unit={frame.player.hybrid ? m.common.percentUnit : undefined} detail={frame.player.hybrid ? m.measured.measured : m.measured.notExposed} />
            <Metric label={m.measured.stateOfCharge} value={frame.player.hybrid ? formatNumber(frame.player.hybrid.stateOfChargePercent, language, 1) : '—'} unit={frame.player.hybrid ? m.common.percentUnit : undefined} detail={frame.player.hybrid ? m.measured.measured : m.measured.notExposed} />
          </div>
        ...
        <Card className="car-state-card">
          <CardHeader eyebrow={m.measured.carEyebrow} title={m.common.tyresBrakes} action={<Badge tone="neutral">{m.measured.rawValues}</Badge>} />
