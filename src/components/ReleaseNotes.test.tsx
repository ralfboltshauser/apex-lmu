...
describe('release-note UI', () => {
  ...
  it('renders a modal dialog, traps focus, handles Escape, and changes language immediately', async () => {
    ...
    expect(container.textContent).toContain('A Garage that fails safely')
    const buttons = [...container.querySelectorAll('button')]
    expect(document.activeElement).toBe(buttons[1])
    await act(async () => buttons[0].click())
    expect(container.textContent).toContain('Eine Garage, die sicher fehlschlägt')
  ...
  it('keeps the complete bundled history available offline', async () => {
    ...
    expect(container.textContent).toContain('v0.4.6')
    expect(container.textContent).toContain('v0.4.5')
    expect(container.textContent).toContain('v0.4.4')
    expect(container.textContent).toContain('v0.4.3')
