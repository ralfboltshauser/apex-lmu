...
describe('release-note UI', () => {
  ...
  it('renders a modal dialog, traps focus, handles Escape, and changes language immediately', async () => {
    ...
    expect(container.textContent).toContain('Durable lap analysis and private feedback')
    const buttons = [...container.querySelectorAll('button')]
    expect(document.activeElement).toBe(buttons[1])
    await act(async () => buttons[0].click())
    expect(container.textContent).toContain('Dauerhafte Rundenanalyse und privates Feedback')
  ...
  it('keeps the complete bundled history available offline', async () => {
    ...
    expect(container.textContent).toContain('v0.2.5')
