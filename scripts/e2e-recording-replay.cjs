...
async function main() {
  ...
  try {
    ...
    await checkpoint('transient lap replay analysis did not load', page.locator('#measured-analysis-tab-debrief').waitFor({ state: 'visible', timeout: 10000 }))
    await page.locator('#measured-analysis-tab-evidence').click()
    await checkpoint('synchronized analysis trace did not load', page.locator('#measured-analysis-panel-evidence .measured-trace polyline.throttle').waitFor({ state: 'visible', timeout: 10000 }))
    ...
    await page.locator('#measured-analysis-tab-evidence').click()
    await checkpoint('imported synchronized trace did not load', page.locator('#measured-analysis-panel-evidence .measured-trace polyline.throttle').waitFor({ state: 'visible', timeout: 10000 }))
    ...
    await restartedPage.locator('#measured-analysis-tab-evidence').click()
    await checkpoint('durable imported trace did not survive restart', restartedPage.locator('#measured-analysis-panel-evidence .measured-trace polyline.throttle').waitFor({ state: 'visible', timeout: 10000 }))
