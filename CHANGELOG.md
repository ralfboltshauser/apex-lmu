...
Strategy can now adopt the current official live session for a fuel-only plan, use measured clean-lap evidence and current race progress, and replan only when accepted evidence or visible manual assumptions change.

- **Adopt measured live facts without inventing a race** — Manual and Live session modes remain separate. Live mode uses current fuel, tank capacity, completed distance, scheduled laps or time, and eligible clean fuel and pace samples. Missing evidence stays unavailable, and session type remains explicitly unknown because LMU's official mapping does not expose it authoritatively.
- **Move the modeled stop when evidence crosses a boundary** — A shared deterministic fuel model now builds fixed-lap and timed-session candidates from the current in-race position. Current fuel can refresh continuously, while the candidate plan changes only for an accepted clean lap, refuel, distance change, session reset or validated manual assumption; a causal card names the accepted change.
- **Keep pit stops coupled to timed race distance** — Timed planning includes current-lap progress and models each pit cost at an actual line crossing, so a stop can correctly change which finish crossing remains reachable. Every candidate card, timeline, pit row, target and driver brief is rendered from the same immutable result.
- **Verify replay isolation and responsive states** — Deterministic boundary and randomized invariant tests cover fractional progress, refuels and feasible multi-stop plans. English and German fixed, timed, unavailable, paused and keyboard-focus states were rendered and visually inspected through 200% scaling. Strict replay of the private 1.741 GB recording normalized all 422,467 frames while producing zero live-model revisions or samples.

**Known limitations**

- **This is a fuel-only plan** — Virtual Energy, tyre inventory and degradation, weather crossover, opponent traffic and rejoin position, driver rules and swap timing are not modeled. Manual pit-lane loss and refuel rate are assumptions, and Apex does not configure the LMU pit menu or command a stop.
- **Live mode is explicit, not automatic race detection** — Apex cannot authoritatively identify practice, qualifying or race from the supported mapping and will not infer it from circumstantial values. The user must explicitly select Live session mode, and a plan remains unavailable until the required measured evidence exists.

### Deutsch — Ein ausdrücklicher adaptiver Live-Kraftstoffplan

Die Strategie kann jetzt die aktuelle offizielle Live-Session für einen reinen Kraftstoffplan übernehmen, gemessene Belege aus sauberen Runden und den aktuellen Rennfortschritt verwenden und nur bei akzeptierten Belegen oder sichtbaren manuellen Annahmen neu planen.

- **Übernimm gemessene Live-Fakten, ohne ein Rennen zu erfinden** — Manueller Modus und Live-Session bleiben getrennt. Der Live-Modus nutzt aktuellen Kraftstoff, Tankkapazität, absolvierte Distanz, geplante Runden oder Zeit sowie geeignete saubere Kraftstoff- und Tempomesswerte. Fehlende Belege bleiben nicht verfügbar, und der Sessiontyp bleibt ausdrücklich unbekannt, weil ihn die offizielle LMU-Abbildung nicht verbindlich bereitstellt.
- **Verschiebe den modellierten Stopp, wenn Belege eine Grenze überschreiten** — Ein gemeinsames deterministisches Kraftstoffmodell erstellt jetzt Kandidaten für feste Runden und Zeitrennen aus der aktuellen Position. Der aktuelle Kraftstoff kann laufend aktualisiert werden; der Kandidatenplan ändert sich nur nach einer akzeptierten sauberen Runde, Betankung, Distanzänderung, Sessionrücksetzung oder geprüften manuellen Annahme. Eine Ursachenkarte nennt die akzeptierte Änderung.
- **Kopple Boxenstopps an die Distanz im Zeitrennen** — Die Zeitplanung berücksichtigt den Fortschritt der aktuellen Runde und modelliert jeden Boxenzeitverlust an einer tatsächlichen Zieldurchfahrt. So kann ein Stopp korrekt ändern, welche Zieldurchfahrt noch erreichbar ist. Kandidatenkarte, Zeitlinie, Boxenzeile, Ziele und Fahrerbriefing stammen jeweils aus demselben unveränderlichen Ergebnis.
- **Prüfe Wiedergabe-Isolation und responsive Zustände** — Deterministische Grenz- und randomisierte Invariantentests decken Teilrundenfortschritt, Betankungen und machbare Mehrstopppläne ab. Englische und deutsche Zustände für feste Runden, Zeitrennen, nicht verfügbare Daten, pausiertes Lernen und Tastaturfokus wurden bis 200 % gerendert und visuell geprüft. Die strikte Wiedergabe der privaten 1,741-GB-Aufzeichnung normalisierte alle 422.467 Frames und erzeugte null Revisionen oder Messwerte im Live-Modell.

**Bekannte Einschränkungen**

- **Dies ist ein reiner Kraftstoffplan** — Virtuelle Energie, Reifenbestand und -abbau, Wetter-Crossover, Gegnerverkehr und Rückkehrposition sowie Fahrerregeln und Wechselzeit werden nicht modelliert. Manuelle Boxengassenzeit und Betankungsrate sind Annahmen; Apex konfiguriert weder das LMU-Boxenmenü noch befiehlt es einen Stopp.
- **Der Live-Modus ist ausdrücklich, keine automatische Rennerkennung** — Apex kann Training, Qualifying oder Rennen mit der unterstützten Abbildung nicht verbindlich erkennen und leitet den Typ nicht aus Indizien ab. Der Benutzer muss den Modus Live-Session ausdrücklich wählen; ein Plan bleibt nicht verfügbar, bis die erforderlichen gemessenen Belege vorliegen.
