...
- **Keep every completed lap available** — Analysis stores full-resolution eligible lap payloads locally, restores them after restart, defaults to a useful completed lap, and keeps earlier laps selectable when the final lap is only partial or telemetry pauses.
...
- **Behalte jede abgeschlossene Runde verfügbar** — Die Analyse speichert vollständige geeignete Rundendaten lokal, stellt sie nach einem Neustart wieder her, wählt standardmäßig eine nützliche abgeschlossene Runde und hält frühere Runden auswählbar, wenn die letzte Runde nur teilweise gefahren wurde oder die Telemetrie pausiert.
