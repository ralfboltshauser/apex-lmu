...
Run commands from the Apex repository root. The CLI reads `APEX_FEEDBACK_API_URL` and `APEX_FEEDBACK_ADMIN_TOKEN` from the environment; `npm run feedback` also loads the ignored root and feedback-API `.env.local` files when present.
